<template>
	<view class="role-page">
		<head-return :toPage="0" title="安徒生故事"></head-return>
		<view class="page-content">
            <view v-for="(content, contentIndex) in contentList" :key="contentIndex + 'content'">
                <view
                    ...
                    <!-- 加载状态 -->
                    <image class="content-item-bt loading-icon" src="/static/icon/item-loading.svg" v-if="playContentIndex === contentIndex && audioState.isLoading"></image>
                    <!-- 播放状态 -->
                    <image class="content-item-bt" src="/static/icon/item-pause.svg" v-else-if="playContentIndex === contentIndex && audioState.isPlaying"></image>
                    <!-- 暂停状态 -->
...
<script>
	...
	export default {
                ...
                contentList: [
                    {
                        title: '小红帽的故事1',
                        featured: true,
                        audioUrl: 'https://web-ext-storage.dcloud.net.cn/uni-app/ForElise.mp3'
                    },
                    {
                        title: '小红帽的故事2',
                        featured: false,
		...
		computed: {
			// 当前播放的音频URL
			currentAudioUrl() {
				if (this.playContentIndex >= 0 && this.contentList[this.playContentIndex]) {
					return this.contentList[this.playContentIndex].audioUrl;
				}
				return '';
			}
		},
